...
{
    [Header("타겟 설정")]
    public Transform target; // 카메라가 따라다닐 대상 (예: 플레이어 캐릭터)

    [Header("카메라 거리 설정")]
    public float distance = 8.0f; // 타겟으로부터의 거리
    public float height = 5.0f; // 타겟으로부터의 높이

    [Header("마우스 설정")]
    public float mouseSensitivity = 2.0f; // 마우스 회전 민감도
    public float minVerticalAngle = -30.0f; // 수직 회전 최소 각도 (아래를 보는 각도)
    public float maxVerticalAngle = 60.0f; // 수직 회전 최대 각도 (위를 보는 각도)

    [Header("부드러움 설정")]
    public float positionSmoothTime = 0.2f; // 위치 따라가기 부드러움 (SmoothDamp에 사용)
    public float rotationSmoothTime = 0.1f; // 회전 부드러움 (SmoothDamp에 사용)
